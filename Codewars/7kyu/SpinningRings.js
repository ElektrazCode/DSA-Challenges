...
spinningRings(3, 2) = 2*/
/**PREP
 * Parameters: 2 numbers representing the max numbers of both rings
 * Return: the number of moves needed until both rings reach the same number
 * Examples: (2, 3) => 5
 * Pseudo-code: while both numbers are not equal
                keep increasing one and descreasing the other % its max
                return number of either one
 */
                function spinningRings(innerMax, outerMax) {
                    ...
                    while(--inner !== ++outer){
                      if (inner===0) inner = innerMax + 1;
                      if (outer===outerMax) outer = -1;
                      counter++;
                    };
                    return counter;
                  };