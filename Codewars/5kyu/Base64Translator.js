...
function base64toBase10(base64) {
    ...
    for(let i=0; i<base64.length; i++){
      let position = base64.length-i-1;
      base10 += code.indexOf(base64[i])*Math.pow(64,position);
    }
    //return base64.split('').reduce((acc, curr, index) => acc += code.indexOf(curr)*Math.pow(64,base64.length-index-1));
  return base10;
}