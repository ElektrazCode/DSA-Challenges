...
function bestMatch(ALAHLYGoals, zamalekGoals) {
  ...
  //   const goalDiff = ALAHLYGoals.map((goalDiff,index)=> ALAHLYGoals[index]-zamalekGoals[index]);
  //   let minDiff = Math.min(...goalDiff);
    
  //   if (goalDiff.filter(goals => goals === minDiff).length>1){
      
    for(let i=0; i<ALAHLYGoals.length; i++){
      if(ALAHLYGoals[i]-zamalekGoals[i]<minDiff){
        minDiff = ALAHLYGoals[i]-zamalekGoals[i];
        index = i;
      }
      else if (ALAHLYGoals[i]-zamalekGoals[i]===minDiff){
