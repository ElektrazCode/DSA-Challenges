...
function dec2FactString(nb) {
    ...
    while(nb>0){
      digit = nb % counter;
      str += (digit>9) ? String.fromCharCode(counter+54) : digit;
  ...
  function factString2Dec(str) {
    ...
    for(let i=0,j=size-1; i<size; i++,j--){
        number = (/[A-Z]/.test(number)) ? number.charCodeAt(0) - 54 : number;
        let fact = factorial(j); 
        number += (+str[i])*fact;
        console.log(`i: ${i}, j: ${j}, fact: ${fact}, number: ${number}`);
      }
      return number;
  }
