 ...
 **/
 function whiteBlackAreas(cols, rows) {
    ...
    let colEvens = cols.filter((num, index) => index % 2 === 0).reduce((sum, num) => sum += num, 0);
    let colOdds = cols.filter((num, index) => index % 2 !== 0).reduce((sum, num) => sum += num, 0);
    let rowEvens = rows.filter((num, index) => index % 2 === 0).reduce((sum, num) => sum += num, 0);
    let rowOdds = rows.filter((num, index) => index % 2 !== 0).reduce((sum, num) => sum += num, 0);
  
    let whites = (colEvens * rowEvens) + (colOdds * rowOdds);
    let blacks = (colOdds * rowEvens) + (colEvens * rowOdds);
