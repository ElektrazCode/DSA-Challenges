...
function sqInRect(lng, wdth){
    ...
    let squares = [];
    let area = lng * wdth;
    let remainLen = Math.max(lng, wdth);
    let remainWid = Math.min(lng, wdth);
  
    while (area > 0){
      area -= remainWid*remainWid;
      squares.push(remainWid);
      remainWid = Math.min(remainLen - remainWid, remainWid);
      remainLen = Math.max(remainLen - remainWid, remainWid);
    }
    return squares;
  }