...
function sumDigNthTerm(initval, patternl, nthterm) {
    let sum = initval;
    let factor = Math.floor(nthterm / patternl.length);
    let extraSteps = nthterm % patternl.length -1; 
    for(let i=0; i< patternl.length; i++){ 
      sum += patternl[i]*factor;
    }
    if (extraSteps === -1) sum -= patternl[patternl.length-1];
    else for(let i=0; i< extraSteps; i++){
      sum += patternl[i];
    }
    return eval(String(sum).split('').join('+'));
  }